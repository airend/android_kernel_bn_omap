...
static int omap_tiler_alloc_dynamicpages(struct omap_tiler_info *info)
{
	...
	for (i = 0; i < info->n_phys_pages; i++) {
		if (!dma_alloc_attrs(NULL, PAGE_SIZE, info->phys_addrs+i,
				GFP_KERNEL | GFP_HIGHUSER, &attrs)) {
			ret = -ENOMEM;
...
err_page_alloc:
	for (i -= 1; i >= 0; i--)
		dma_free_attrs(NULL, PAGE_SIZE, dma_to_virt(NULL, info->phys_addrs[i]),
...
static void omap_tiler_free_dynamicpages(struct omap_tiler_info *info)
{
	...
	for (i = info->n_phys_pages - 1; i >= 0; i--) {
		...
		if (!token)
			continue;
		dma_free_attrs(NULL, PAGE_SIZE, token, info->phys_addrs[i], &attrs);
	}
	return;
}
